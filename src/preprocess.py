...
def normalize(image, thresh):
    numerator = (image - np.min(image))
    denominator = (np.max(image) - np.min(image))
    norm_image = numerator / denominator

    while np.gradient(norm_image, image) > thresh:
        numerator = (image - np.min(image))
        denominator = (np.max(image) - np.min(image))
        norm_image = numerator / denominator
...
def standardize(image):
    ...
    standardized_image = datagen.flow(image)
    return standardized_image

class ImageSplitter:
    # key points:
    # Border padding and over-lapping img splitting to avoid the instability of edge value
