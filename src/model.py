...
# singleton pattern applied for
class Generator(nn.Module):

    instance = None

    @classmethod
    def get_instance(cls, input_dim=3, dim=64):
        if not cls.instance:
            return cls(input_dim, dim)
        return cls.instance
    ...
    def __init__(self, input_dim=3, dim=64):
        ...
        self.feat_out_1 = AuxLayer(input_size=dim, output_size=dim, kernel_size=3, stride=1, padding=1)
        self.feat_out_2 = AuxLayer(input_size=2 * dim, output_size=dim, kernel_size=3, stride=1,
                                   padding=1)
        self.feat_out_3 = AuxLayer(input_size=3 * dim, output_size=dim, kernel_size=3, stride=1,
                                   padding=1)
